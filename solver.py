...
def PGD_attack(model, benign_img, delta, start_class, target_class, loss, steps, epsilon, ratio, mask = None):
    ...
    if mask is None:
        ...
        # mask = normalize_image(grads[0]).numpy()<0.1
    ...
    for step in range(0, steps):
        ...
        with tf.GradientTape() as tape:
            ...
            # Tracks the gradients of <delta>
            tape.watch(delta)
            
            # Generates a tentative adversarial example
            adv_example = benign_img + ratio * mask * delta

            # adv_img = unprocess(adv_example, 224, 224)
            write_to_disk(adv_example[0], f"hotdog_check@generator_{check}.tiff")
            write_to_disk(adv_example[0], f"hotdog_check@generator_{check}.png")
            l2_distance, diff_pixels = compare_to_hotdog(f"hotdog_check@generator_{check}.tiff")
            
            # if l2_distance >= 2.6 or diff_pixels.mean() >= 0.005 or diff_pixels.max() >= 0.1:
            #     print("fail -- ", l2_distance, diff_pixels.mean(), diff_pixels.max())
            #     return True

            # Predicts the adversarial example's output through the model.
            # Computes the resulting loss s.t. backprop will update <delta> to move <benign_img>
            # away from the original class and towards the target class
            predictions   = model(adv_example, training=False)
            original_loss = -loss(tf.convert_to_tensor([start_class]), predictions)
            target_loss   = loss(tf.convert_to_tensor([target_class]), predictions)
            l2_loss       = 100*keras.losses.MeanSquaredError()(benign_img, adv_example)
            print(l2_loss)

            if predictions[0, target_class] > 0.99:
                write_to_disk(adv_example[0], f"hotdog_check@end_{check}.tiff")
                write_to_disk(adv_example[0], f"hotdog_check@end_{check}.png")
                write_to_disk(adv_example[0]-benign_img[0], f"hotdog_check@diff_{check}.tiff")
                l2_distance, diff_pixels = compare_to_hotdog(f"hotdog_check@end_{check}.tiff")
                print(f"final step: {step},  conf: {predictions[0, target_class]:.4}, loss: {total_loss.numpy():.2}, l2: {l2_distance.mean():.4}, Mean diff: {diff_pixels.mean():.4}, Max diff: {diff_pixels.max():.4}")
                return False
            
            total_loss    = original_loss + target_loss + l2_loss
            print(f"step: {step},  conf: {predictions[0, target_class]:.4}, loss: {total_loss.numpy():.2}, l2: {l2_distance.mean():.4}, Mean diff: {diff_pixels.mean():.4}, Max diff: {diff_pixels.max():.4}")
        
            # Computes the gradient of <delta> w.r.t. the loss
            gradients = tape.gradient(total_loss, delta)
            
            # update <delta> and clips it to a ball of size [-epsilon, -epsilon]
            optimizer.apply_gradients([(gradients, delta)])
            delta.assign_add(clip(delta, epsilon))
    return True
...
while True:
    ...
    val = PGD_attack(model, benign_img, delta, benign_pred_index, flag_label, loss, steps, epsilon, ratio)
    if not val:
        break